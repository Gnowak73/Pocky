...
def _weighted_spatial_moments(x: np.ndarray) -> Dict[str, float]:
    total = float(np.sum(x)) + 1e-12
    h, w = x.shape
    yy = np.linspace(-1.0, 1.0, h)[:, None]
    xx = np.linspace(-1.0, 1.0, w)[None, :]
    cy = float(np.sum(x * yy) / total)
    cx = float(np.sum(x * xx) / total)
    dy = yy - cy
    dx = xx - cx
    var_y = float(np.sum(x * (dy**2)) / total)
    var_x = float(np.sum(x * (dx**2)) / total)
    cov_xy = float(np.sum(x * dx * dy) / total)
    tr = var_x + var_y
    det = max(0.0, var_x * var_y - cov_xy * cov_xy)
    disc = max(0.0, tr * tr - 4.0 * det)
    l1 = 0.5 * (tr + np.sqrt(disc))
    l2 = 0.5 * (tr - np.sqrt(disc))
    elong = float((l1 - l2) / (l1 + l2 + 1e-12))
    return {
        "spatial_cx": cx,
        "spatial_cy": cy,
        "spatial_var_x": var_x,
        "spatial_var_y": var_y,
        "spatial_cov_xy": cov_xy,
        "spatial_radius": float(np.sqrt(max(tr, 0.0))),
        "spatial_elong": elong,
        "spatial_axis_ratio": float(np.sqrt(max(l1, 0.0) / (max(l2, 0.0) + 1e-12))),
    }
...
def frame_features(
    ...
    amp_q50, amp_q75, amp_q90, amp_q95, amp_q99 = [float(q) for q in np.quantile(amp_flat, [0.50, 0.75, 0.90, 0.95, 0.99])]
    amp_iqr = float(amp_q75 - amp_q50)
    amp_cv = _safe_ratio(amp_std, amp_mean)

    # average over channels for spectral features
    x_mean = amp.mean(axis=0)
    x_total = float(np.sum(x_mean)) + 1e-12
    ...
    for c in range(amp.shape[0]):
        ch_means.append(float(np.mean(amp[c])))
        ch_stds.append(float(np.std(amp[c])))
        _, c_centroid, _, _, _, _, _ = spectral_features(amp[c])
        ch_centroids.append(float(c_centroid))
    if ch_centroids:
        ch_mean_arr = np.array(ch_means, dtype=np.float64)
        ch_std_arr = np.array(ch_stds, dtype=np.float64)
        ...
        amp_ch_mean = float(np.mean(ch_mean_arr))
        amp_ch_std = float(np.std(ch_mean_arr))
        amp_ch_max = float(np.max(ch_mean_arr))
        amp_ch_min = float(np.min(ch_mean_arr))
        amp_ch_cv = _safe_ratio(amp_ch_std, amp_ch_mean)
        tex_ch_mean = float(np.mean(ch_std_arr))
        tex_ch_std = float(np.std(ch_std_arr))
    else:
        centroid_ch_mean = centroid
        centroid_ch_std = 0.0
        centroid_ch_max = centroid
        centroid_ch_min = centroid
        amp_ch_mean = amp_mean
        amp_ch_std = 0.0
        amp_ch_max = amp_mean
        amp_ch_min = amp_mean
        amp_ch_cv = 0.0
    ...
    grad_q90, grad_q99 = [float(q) for q in np.quantile(grad.reshape(-1), [0.90, 0.99])]
    gx_abs_mean = float(np.mean(np.abs(gx)))
    gy_abs_mean = float(np.mean(np.abs(gy)))
    lap = (
    ...
    lap_abs_mean = float(np.mean(np.abs(lap)))

    x_q50, x_q75, x_q90, x_q95, x_q99 = [float(q) for q in np.quantile(x_mean.reshape(-1), [0.50, 0.75, 0.90, 0.95, 0.99])]
    mask95 = x_mean >= x_q95
    mask99 = x_mean >= x_q99
    hot_frac95 = float(np.mean(mask95))
    hot_frac99 = float(np.mean(mask99))
    hot_energy95 = _safe_ratio(float(np.sum(x_mean[mask95])), x_total)
    hot_energy99 = _safe_ratio(float(np.sum(x_mean[mask99])), x_total)
    top5_count = max(1, int(0.05 * x_mean.size))
    top10_count = max(1, int(0.10 * x_mean.size))
    x_sorted = np.sort(x_mean.reshape(-1))
    top5_energy = _safe_ratio(float(np.sum(x_sorted[-top5_count:])), x_total)
    top10_energy = _safe_ratio(float(np.sum(x_sorted[-top10_count:])), x_total)

    spatial = _weighted_spatial_moments(x_mean)
    yy = np.linspace(-1.0, 1.0, x_mean.shape[0])[:, None]
    xx = np.linspace(-1.0, 1.0, x_mean.shape[1])[None, :]
    rr = np.sqrt(xx * xx + yy * yy)
    inner = float(np.sum(x_mean[rr <= 0.33]))
    mid_ring = float(np.sum(x_mean[(rr > 0.33) & (rr <= 0.66)]))
    outer = float(np.sum(x_mean[rr > 0.66]))
    horiz = float(np.sum(x_mean[:, x_mean.shape[1] // 3 : (2 * x_mean.shape[1]) // 3]))
    vert = float(np.sum(x_mean[x_mean.shape[0] // 3 : (2 * x_mean.shape[0]) // 3, :]))
    ...
    feats = {
        ...
        "amp_q99": amp_q99,
        "amp_iqr": amp_iqr,
        "amp_q95_q50": _safe_ratio(amp_q95, amp_q50),
        "amp_q99_q90": _safe_ratio(amp_q99, amp_q90),
        ...
        "amp_ch_mean": amp_ch_mean,
        "amp_ch_std": amp_ch_std,
        "amp_ch_max": amp_ch_max,
        "amp_ch_min": amp_ch_min,
        "amp_ch_cv": amp_ch_cv,
        "tex_ch_mean": tex_ch_mean,
        "tex_ch_std": tex_ch_std,
        ...
        "spec_low_frac": _safe_ratio(low, low + mid + high),
        "spec_mid_frac": _safe_ratio(mid, low + mid + high),
        "spec_high_frac": _safe_ratio(high, low + mid + high),
        "spec_mid_low": _safe_ratio(mid, low),
        "spec_high_mid": _safe_ratio(high, mid),
        ...
        "nested_low_frac": _safe_ratio(nested_low, nested_low + nested_mid + nested_high),
        "nested_mid_frac": _safe_ratio(nested_mid, nested_low + nested_mid + nested_high),
        "nested_high_frac": _safe_ratio(nested_high, nested_low + nested_mid + nested_high),
        "nested_high_mid": _safe_ratio(nested_high, nested_mid),
        ...
        "hot_frac95": hot_frac95,
        "hot_frac99": hot_frac99,
        "hot_energy95": hot_energy95,
        "hot_energy99": hot_energy99,
        ...
        "img_q95": x_q95,
        "img_q99": x_q99,
        "img_inner_frac": _safe_ratio(inner, x_total),
        "img_mid_frac": _safe_ratio(mid_ring, x_total),
        "img_outer_frac": _safe_ratio(outer, x_total),
        "img_outer_inner": _safe_ratio(outer, inner),
        "img_horiz_frac": _safe_ratio(horiz, x_total),
        "img_vert_frac": _safe_ratio(vert, x_total),
        **spatial,
        "grad_mean": grad_mean,
        "grad_std": grad_std,
        "grad_q90": grad_q90,
        "grad_q99": grad_q99,
        "grad_x_mean": gx_abs_mean,
        "grad_y_mean": gy_abs_mean,
        "grad_xy_ratio": _safe_ratio(gx_abs_mean, gy_abs_mean),
        "lap_energy": lap_energy,
        "lap_abs_mean": lap_abs_mean,
    }
    ...
    if prev is not None:
        ...
        feats["delta_energy"] = float(np.mean(delta_abs))
        feats["max_delta"] = float(np.max(delta_abs))
        feats["delta_std"] = float(np.std(delta_abs))
        feats["delta_q95"] = float(np.quantile(delta_abs.reshape(-1), 0.95))
        feats["delta_rel_energy"] = _safe_ratio(feats["delta_energy"], amp_mean)
    else:
        feats["delta_energy"] = 0.0
        feats["max_delta"] = 0.0
        feats["delta_std"] = 0.0
        feats["delta_q95"] = 0.0
        feats["delta_rel_energy"] = 0.0
    ...
    if vis_c is not None:
        ...
        feats["phase_grad_std"] = float(pgrad.std())
        feats["phase_grad_q95"] = float(np.quantile(pgrad.reshape(-1), 0.95))
        ...
        if prev_c is not None:
            ...
            feats["phase_delta_std"] = float(np.std(delta_phase))
            feats["phase_delta_q95"] = float(np.quantile(np.abs(delta_phase).reshape(-1), 0.95))
            feats["phase_jump"] = float(np.mean(np.abs(delta_phase) > (0.5 * np.pi)))
        else:
            feats["temp_coh"] = 0.0
            feats["phase_delta_mean"] = 0.0
            feats["phase_delta_std"] = 0.0
            feats["phase_delta_q95"] = 0.0
