...
_RUNTIME_GOES_INTERVAL_MAX_BASE_COLS = {
    "fai_T_in_6_20",
    "fai_T_in_7_14",
    "fai_EM_gt_005",
    "fai_EM_gt_01",
...
def _build_runtime_goes_aux_row(
    ...
    if not cols:
        return row
    def _interval_reduce(base: str, values: pd.Series) -> float:
        vals = pd.to_numeric(values, errors="coerce").to_numpy(dtype=np.float64)
        vals = vals[np.isfinite(vals)]
        if vals.size == 0:
            return np.nan
        if base in _RUNTIME_GOES_INTERVAL_MAX_BASE_COLS:
    ...
    try:
        ...
        start_ts = target_ts.floor("min")
        if float(aux_lead_min) <= 0.0:
            hit = feat_df.loc[feat_df.index <= start_ts]
            if hit.empty:
                return row
            src = hit.iloc[-1]
        else:
            end_ts = (target_ts + pd.Timedelta(minutes=float(aux_lead_min))).floor("min")
            window = feat_df.loc[(feat_df.index >= start_ts) & (feat_df.index <= end_ts)]
            if window.empty:
                return row
            src = pd.Series(index=window.columns, dtype=np.float64)
            for base in _RUNTIME_GOES_PAPER_BASE_COLS:
                if base in window.columns:
                    agg = _interval_reduce(base, window[base])
                    if np.isfinite(agg):
                        src[base] = agg
        for base in _RUNTIME_GOES_PAPER_BASE_COLS:
...
def _select_runtime_goes_cursor_row(
    ...
    try:
        ...
        if target_ts.tzinfo is None:
            ...
            target_ts = target_ts.tz_convert("UTC")
        target_ts = (target_ts + pd.Timedelta(minutes=float(lead_minutes))).floor("min")
...
def _c5_pretrigger_condition(box_em_total: float, t_mk: float, em49: float) -> bool:
...
def save_flare_trigger_snapshot(
    ...
    snapshot_root_name="Flare Triggers",
    snapshot_label="flare trigger",
):
            ...
            if idx >= 0:
                ...
                if imminence_runtime:
                    ...
                    for i in range(n_ar):
                        ...
                        pretrigger_hit = _c5_pretrigger_condition(
                            float(em_totals[i]), cursor_t_mk, cursor_em49
                        )
                        pretrigger_risk_history[box_label].append(
                            1.0 if pretrigger_hit else 0.0
                        )
                        ...
                        if len(recent_vals) >= int(imminence_alert_count):
                            ...
                            avg_recent = float(np.mean(recent_vals))
                            peak_recent = float(np.max(recent_vals))
                            em_jump_threshold = 1.10 if (
                                np.isfinite(area_frac) and area_frac > 0.06
                            ) else 1.02
                            em_jump_ok = (
                                np.isfinite(em_prev)
                                and np.isfinite(em_curr)
                                and em_prev > 0.0
                                and (em_curr / em_prev) >= em_jump_threshold
                            )
                            large_area_gate = (
                            ...
                            if large_area_gate:
                                avg_ok = avg_recent >= float(imminence_large_area_alert_avg_threshold)
                                peak_ok = peak_recent >= float(imminence_large_area_peak_threshold)
                                em_ok = em_ok and (
                                    np.isfinite(em_prev)
                                    and np.isfinite(em_curr)
                                    and em_prev > 0.0
                                    and (em_curr / em_prev) >= 1.10
                                )
                            triggered = (
                                avg_ok
                                and peak_ok
                            ...
                            pretrigger_ok = (
                                ...
                                and all(v >= 0.5 for v in recent_pretrigger_vals)
                            )
                            if i == focus_idx:
                                box_trigger_cooldown = int(imminence_alert_cooldown_remaining[box_label])
                                box_pretrigger_cooldown = int(
                                    imminence_pretrigger_cooldown_remaining[box_label]
                                )
                                if (
                                    pretrigger_ok
                                    and not triggered
                                    and box_trigger_cooldown == 0
                                    and box_pretrigger_cooldown == 0
                                ):
                    ...
                    if any(trigger_states):
                        ...
                        if (
                            ...
                            or tracked_avg < persistent_pretrigger_release_avg
                        ):
                            persistent_pretrigger_active = False
                            persistent_pretrigger_label = None
                    plot_imminence_risk_history(
                ...
                if cycle_pretrigger_record is not None:
                    (
                        pretrigger_time_utc,
                        pretrigger_time_local,
                        pretrigger_focus_label,
                        pretrigger_focus_em,
                        pretrigger_box_stats,
                    ) = cycle_pretrigger_record
                    save_flare_trigger_snapshot(
                        ...
                        copy_em_goes_plot=True,
                        snapshot_root_name="Flare Pretriggers",
                        snapshot_label="flare pretrigger",
                    )
