...
def download_aia_data(wav, t_rec, segments, data_folder, timezone='US/Central', silent=False):
    ...
    if not silent:
        print("\nStart download AIA data recorded at " + this_datetime_timezone.strftime("%m-%d-%YT%H:%M:%S") + " " + timezone)
...
def extract_submap(aia_map, ar_lon, ar_lat, n_pix_x = 700, n_pix_y = 700):
    ...
    """
    this_coord = SkyCoord(ar_lon*u.deg, ar_lat*u.deg, frame=frames.HeliographicStonyhurst) #frame=aia_map.coordinate_frame; x*u.arcsec and y*u.arcsec in helioprojected

    pix_x = aia_map.world_to_pixel(this_coord).x.value
    pix_y = aia_map.world_to_pixel(this_coord).y.value

    top_right   = aia_map.pixel_to_world((pix_x+n_pix_x//2-1)*u.pix,(pix_y+n_pix_y//2-1)*u.pix)
    bottom_left = aia_map.pixel_to_world((pix_x-n_pix_x//2)*u.pix,(pix_y-n_pix_y//2)*u.pix)
...
def crop_full_disk_maps(aia_maps, ar_lon, ar_lat, arnum, cropped_maps_folder, n_pix_x=1000, n_pix_y=1000, save_submaps=False):
    """
    ...
    for aia_map in aia_maps:
        aia_submap = extract_submap(aia_map, ar_lon, ar_lat, n_pix_x = n_pix_x, n_pix_y = n_pix_y)
...
def plot_full_disk_images(calibrated_aia_maps, plots_folder, t_rec, arnum, ar_lon, ar_lat, color_arr,
                         timezone='US/Central', n_pix_x=1000, n_pix_y=1000):  
#def plot_full_disk_images(calibrated_aia_maps, plots_folder, t_rec, arnum, ar_lon, ar_lat, color_arr,
#                        timezone='US/Central', n_pix_x=1400, n_pix_y=1400): #### new suggested version to make boxes bigger
    ...
    for jj in range(5):
        ...
        for ii in range(len(ar_lon)):
            ...
            top_right   = this_map.pixel_to_world((pix_x+n_pix_x//2-1)*u.pix,(pix_y+n_pix_y//2-1)*u.pix)
            bottom_left = this_map.pixel_to_world((pix_x-n_pix_x//2)*u.pix,(pix_y-n_pix_y//2)*u.pix)
...
def stream_aia_data(duration_stream, data_folder, ar_lon, ar_lat, arnum, label, 
                    correction_table, timezone='US/Central', n_pix_x=1000, n_pix_y=1000, latency=10, 
                    reference_wav=193, th_tot_em=0,
                    weights = [1.20196640e-04,  2.12817313e-05, -7.33613022e-07,  1.83818002e-07, -1.90719161e-06], save_maps=False):
    """
    ...
    while time_diff <= duration_stream:
        ...
        if idx >= 0:
            ...
            # Plot full-disk maps
            plot_full_disk_images(calibrated_aia_maps,plots_folder,grouped_t_rec,arnum,ar_lon,ar_lat,color_arr,timezone=timezone, n_pix_x=n_pix_x, n_pix_y=n_pix_y)
            ...
            for i in range(n_ar):
                ...
                idx = np.where(em_map_th < th_tot_em) # set values below the treshold equal to 0
                em_map_th[idx] = 0
                #write_csv_em(file_name_em_csv, grouped_t_rec[0], np.sum(em_map_th)) # original line
                write_csv_em(file_name_em_csv, grouped_t_rec[0], np.sum(em_map_th)*1.e6/(n_pix_x*n_pix_y)) # new line with variable box size
