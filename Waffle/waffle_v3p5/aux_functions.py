...
def _runtime_state_path(data_folder):
    return os.path.join(data_folder, "waffle_v3_runtime_state.pkl")
...
def resolve_solarmonitor_boxes(
    ...
    selected_arnum = [int(r["region"]) % 10000 for r in selected] + list(
        range(1, fallback_needed + 1)
    )
    selected_x = [float(r["x_arcsec"]) for r in selected] + [float("nan")] * fallback_needed
    selected_y = [float(r["y_arcsec"]) for r in selected] + [float("nan")] * fallback_needed
    selected_priority = [float(r.get("activity_score", 0.0)) for r in selected] + [float("-inf")] * fallback_needed
    while len(selected_arnum) < 6:
        selected_arnum.append(len(selected_arnum) + 1)
...
def _save_runtime_stream_state(
    ...
    risk_history,
    risk10_history,
    pretrigger_risk_history,
    active_area_fraction_history,
    vis_history,
    em_history,
    goes_aux_history,
    imminence_alert_cooldown_remaining,
    imminence_pretrigger_cooldown_remaining,
    persistent_pretrigger_active,
    persistent_pretrigger_label,
):
    state = {
        "labels": list(labels),
        "risk_history": {k: list(v) for k, v in risk_history.items()},
        "risk10_history": {k: list(v) for k, v in risk10_history.items()},
        "pretrigger_risk_history": {
            k: list(v) for k, v in pretrigger_risk_history.items()
        },
        "active_area_fraction_history": {
            k: list(v) for k, v in active_area_fraction_history.items()
        },
        "vis_history": {
            k: [np.asarray(x, dtype=np.float32) for x in v]
            for k, v in vis_history.items()
        },
        "em_history": {k: list(v) for k, v in em_history.items()},
        "goes_aux_history": [dict(x) for x in goes_aux_history],
        "imminence_alert_cooldown_remaining": dict(imminence_alert_cooldown_remaining),
        "imminence_pretrigger_cooldown_remaining": dict(
            imminence_pretrigger_cooldown_remaining
        ),
        "persistent_pretrigger_active": bool(persistent_pretrigger_active),
        "persistent_pretrigger_label": persistent_pretrigger_label,
    }
...
def _load_runtime_stream_state(data_folder, labels):
    ...
    try:
        ...
        print(f"Failed to load waffle_v3 runtime state: {exc}")
        return None
    if list(state.get("labels", [])) != list(labels):
        print("Ignoring waffle_v3 runtime state: box labels do not match current run.")
        return None
    return state
...
def _runtime_compute_temp_em(diff_df: pd.DataFrame) -> pd.DataFrame:
    from sunkit_instruments.goes_xrs.goes_chianti_tem import calculate_temperature_em

    out = pd.DataFrame(index=diff_df.index, data={"fai_T_mk": np.nan, "fai_EM49": np.nan})
    valid = (
        np.isfinite(diff_df["xrsa"].to_numpy())
        & np.isfinite(diff_df["xrsb"].to_numpy())
        & (diff_df["xrsa"].to_numpy() > 0.0)
        & (diff_df["xrsb"].to_numpy() > 0.0)
    )
    if not np.any(valid):
        return out
    valid_df = diff_df.loc[valid, ["xrsa", "xrsb"]].copy()
    observatory = "GOES 18"
    ts = _runtime_make_xrs_timeseries(valid_df, observatory)
    temp_em = calculate_temperature_em(ts).to_dataframe()
    out.loc[temp_em.index, "fai_T_mk"] = pd.to_numeric(
        temp_em["temperature"], errors="coerce"
    ).to_numpy(dtype=np.float64)
    out.loc[temp_em.index, "fai_EM49"] = (
...
def _runtime_safe_ratio(a: pd.Series, b: pd.Series) -> pd.Series:
    den = b.where(np.abs(b) > 0.0, np.nan)
    return a / den
...
def _runtime_compute_goes_paper_feature_frame(goes_df: pd.DataFrame) -> pd.DataFrame:
    ...
    temp_em = _runtime_compute_temp_em(diff_for_tem)
    df = pd.concat([df, temp_em], axis=1)
    df["fai_T_in_6_20"] = ((df["fai_T_mk"] > 6.0) & (df["fai_T_mk"] < 20.0)).astype(np.float64)
    df["fai_T_in_7_14"] = ((df["fai_T_mk"] > 7.0) & (df["fai_T_mk"] < 14.0)).astype(np.float64)
    ...
    df["fai_consec_3_005"] = (consec_005 >= 3).astype(np.float64)
    df["fai_consec_2_01"] = (consec_01 >= 2).astype(np.float64)
    df["fai_consec_3_01"] = (consec_01 >= 3).astype(np.float64)
    return df


def _build_runtime_goes_aux_row(
    xrsa_current,
    xrsb_current,
    anchor_time_ut,
    aux_prefix: str,
    ...
    cols = [f"{aux_prefix}_{c}" for c in _RUNTIME_GOES_PAPER_BASE_COLS]
    row = {c: 0.0 for c in cols}
    row.update({f"{c}_present": 0.0 for c in cols})
    if not cols:
        return row
    try:
        ...
        target_ts = (target_ts + pd.Timedelta(minutes=float(aux_lead_min))).floor("min")
        hit = feat_df.loc[feat_df.index <= target_ts]
        if hit.empty:
            return row
        src = hit.iloc[-1]
        for base in _RUNTIME_GOES_PAPER_BASE_COLS:
            dst = f"{aux_prefix}_{base}"
            val = pd.to_numeric(src.get(base, np.nan), errors="coerce")
            if np.isfinite(val):
                row[dst] = float(val)
                row[f"{dst}_present"] = 1.0
        return row
    except Exception as exc:
        print(f"Runtime GOES/FAI feature build failed: {exc}")
        return row


def _merge_goes_aux_history(feats_list, goes_aux_history, goes_aux_cols):
    if not feats_list or not goes_aux_cols:
        return feats_list
    zero_row = {c: 0.0 for c in goes_aux_cols}
    if not goes_aux_history:
        return [dict(f, **zero_row) for f in feats_list]
    src = list(goes_aux_history)
    if len(src) < len(feats_list):
        src = ([zero_row] * (len(feats_list) - len(src))) + src
    else:
        src = src[-len(feats_list):]
    out = []
    for feats, aux in zip(feats_list, src):
        row = dict(feats)
        for col in goes_aux_cols:
            row[col] = float(aux.get(col, 0.0))
        out.append(row)
    return out
...
def _load_imminence_runtime(model_path, state_model_path="", cache_slot="main"):
    global _IMMINENCE_RUNTIME, _PRETRIGGER_IMMINENCE_RUNTIME
    if cache_slot == "pretrigger":
        def _get_runtime():
            return _PRETRIGGER_IMMINENCE_RUNTIME

        def _set_runtime(val):
            global _PRETRIGGER_IMMINENCE_RUNTIME
            _PRETRIGGER_IMMINENCE_RUNTIME = val
            return val
    else:
        def _get_runtime():
    ...
    if cached is not None:
        return cached
    if not model_path:
        raise RuntimeError("waffle_v3 requires an imminence model path.")

    if state_model_path:
        raise RuntimeError("waffle_v3 does not support a separate state model.")
    ...
    if not os.path.exists(uv_grid_path):
        raise FileNotFoundError(f"waffle_v3 UV grid not found: {uv_grid_path}")
    ...
    if cache_slot == "main":
        if not _is_legacy_main_model_name(model_name):
            raise RuntimeError(
                f"waffle_v3 main model must be a legacy_main_trigger*.pt checkpoint, got {model_name}"
            )
        feature_family = "legacy"
    else:
    ...
    aux_csv = str(args.get("aux_csv", "") or "").strip()
    raw_goes_aux_cols = []
    if cache_slot == "main" and any(name.startswith("goes_") for name in feature_names):
        raw_goes_aux_cols = [f"{aux_prefix}_{c}" for c in _RUNTIME_GOES_PAPER_BASE_COLS]
        raw_goes_aux_cols.extend([f"{c}_present" for c in list(raw_goes_aux_cols)])
        feature_family = f"legacy_goes_paper_{len(feature_names)}"
    ...
    runtime = {
        **base_runtime,
        ...
        "goes_aux_cols": raw_goes_aux_cols,
        "goes_aux_prefix": aux_prefix,
        "goes_aux_lead_min": float(args.get("aux_lead_min", 0.0) or 0.0),
        "goes_aux_csv": aux_csv,
    }
    print(
        "Imminence model enabled: "
        f"{model_path} (family={feature_family}, warmup={runtime['warmup']}, bins={bins}, seq_len={runtime['seq_len']})"
    )
    return _set_runtime(runtime)
...
def _start_imminence_worker(model_path, state_model_path, base_runtime):
    ...
    if state_model_path:
        cmd.extend(["--state-model", state_model_path])
    try:
...
def _infer_imminence_risk_from_history(vis_history, runtime, em_history=None, goes_aux_history=None):
    ...
    if runtime.get("external_worker"):
        ...
        try:
            ...
            np.savez_compressed(
                tmp_path,
                vis=np.asarray(vis_history, dtype=np.float32),
                em=np.asarray(em_history or [], dtype=np.float64),
                goes_aux=np.asarray(
                    [
                        [float(row.get(col, 0.0)) for col in runtime.get("goes_aux_cols", [])]
                        for row in (goes_aux_history or [])
                    ],
                    dtype=np.float32,
                ),
            )
    ...
    cache_key = (
        ...
        len(goes_aux_history or []),
        id(goes_aux_history[-1]) if goes_aux_history else 0,
        int(runtime["warmup"]),
    ...
    if cached is None:
        ...
        for vis in vis_history:
            ...
            prev = vis
        feats_list = _merge_goes_aux_history(
            feats_list, goes_aux_history, runtime.get("goes_aux_cols", [])
        )
...
def _extract_10m_probability(prob_val, bins) -> float:
    if prob_val is None:
        return float("nan")
    bins_arr = [float(x) for x in bins]
    try:
        idx = bins_arr.index(10.0)
    except ValueError:
        if not bins_arr:
            return float("nan")
        idx = int(np.argmin(np.abs(np.asarray(bins_arr, dtype=np.float64) - 10.0)))
    flat = np.asarray(prob_val, dtype=np.float32).ravel()
...
def plot_imminence_risk_history(
    ...
    color_arr,
    label,
    avg_trigger_threshold,
    small_area_alert_threshold=0.60,
    large_area_alert_threshold=None,
    focus_label=None,
):
    ...
    if (
        small_area_alert_threshold is not None
        and float(small_area_alert_threshold) != float(avg_trigger_threshold)
    ):
        ax.axhline(
            float(small_area_alert_threshold),
            color="dimgray",
            linestyle="--",
            linewidth=1,
            label="_nolegend_",
    ...
    if (
        large_area_alert_threshold is not None
        and float(large_area_alert_threshold) != float(avg_trigger_threshold)
        and (
            small_area_alert_threshold is None
            or float(large_area_alert_threshold) != float(small_area_alert_threshold)
        )
    ):
        ax.axhline(
            float(large_area_alert_threshold),
            color="firebrick",
            linestyle="--",
            linewidth=1,
            label="_nolegend_",
...
def prepare_goes_plot_arrays(
    ...
    if anchor_time_local is not None and len(goes_time_array) > 0:
        min_time = anchor_time_local - timedelta(minutes=float(lookback_minutes))
        max_time = anchor_time_local + timedelta(minutes=float(lead_minutes))
...
def _latest_xrsb_derivative(xrsb_current):
    if xrsb_current is None or len(xrsb_current) < 2:
        return np.nan
    try:
        flux = pd.to_numeric(xrsb_current["flux"], errors="coerce").to_numpy(dtype=np.float64)
    except Exception:
        return np.nan
...
def _fetch_goes_day_dataframe(day: datetime.date, download_root: Path) -> pd.DataFrame:
    ...
    if len(result) == 0:
        raise FileNotFoundError(f"no GOES XRS search results for {day.isoformat()}")
    files = client.fetch(
        result,
    ...
    if len(files) == 0:
        raise FileNotFoundError(f"GOES XRS fetch returned no files for {day.isoformat()}")
    ts = TimeSeries(files, concatenate=True)
    df = ts.to_dataframe()
    if df is None or len(df) == 0:
        raise FileNotFoundError(f"GOES XRS dataframe empty for {day.isoformat()}")
    cols = {str(c).lower(): c for c in df.columns}
    col_a = cols.get("xrsa")
    col_b = cols.get("xrsb")
    if col_a is None or col_b is None:
        raise ValueError(f"GOES XRS dataframe missing XRSA/XRSB columns for {day.isoformat()}")
    ...
    dfa = dfa[ok]
    dfb = dfb[ok]
    in_day = (idx >= pd.Timestamp(start_time)) & (idx <= pd.Timestamp(end_time))
    idx = idx[in_day]
    dfa = dfa[in_day]
    dfb = dfb[in_day]
    if len(idx) == 0:
        raise FileNotFoundError(f"GOES XRS valid rows empty for {day.isoformat()}")
    return pd.DataFrame(
...
def _ensure_local_goes_cache_window(goes_folder, start_time, end_time):
    ...
    while cur <= end_time.date():
        ...
        if not day_path.exists():
            df_day = _fetch_goes_day_dataframe(cur, download_root)
            _save_goes_cache_day(cache_root, cur, df_day)
            built_days.append(cur)
        cur += timedelta(days=1)
    return cache_root, built_days


def load_archive_XRS(reference_time_ut, goes_folder, lookback_hours=6, lead_minutes=4.0):
    """
...
def stream_aia_data(
    ...
    imminence_pretrigger_model_path="",
    ...
    imminence_pretrigger_avg10_threshold=0.80,
    imminence_pretrigger_em_jump_ratio=1.10,
    ...
    imminence_pretrigger_cooldown_frames=10,
    ...
    imminence_large_area_peak_threshold=0.90,
    imminence_large_area_em_ratio_threshold=1.02,
    ...
    risk10_history = {lab: [] for lab in label[:n_ar]}
    pretrigger_risk_history = {lab: [] for lab in label[:n_ar]}
    ...
    imminence_pretrigger_cooldown_remaining = {lab: 0 for lab in label[:n_ar]}
    persistent_pretrigger_active = False
    persistent_pretrigger_label = None
    persistent_pretrigger_release_avg = 0.45
    ...
    if resumed_state is not None:
        risk_history.update(resumed_state.get("risk_history", {}))
        risk10_history.update(resumed_state.get("risk10_history", {}))
        pretrigger_risk_history.update(
            resumed_state.get("pretrigger_risk_history", {})
        )
        active_area_fraction_history.update(
            resumed_state.get("active_area_fraction_history", {})
        )
        vis_history.update(resumed_state.get("vis_history", {}))
        em_history.update(resumed_state.get("em_history", {}))
        goes_aux_history = list(resumed_state.get("goes_aux_history", []))
        imminence_alert_cooldown_remaining.update(
            resumed_state.get("imminence_alert_cooldown_remaining", {})
        )
        imminence_pretrigger_cooldown_remaining.update(
            resumed_state.get("imminence_pretrigger_cooldown_remaining", {})
        )
        persistent_pretrigger_active = bool(
            resumed_state.get("persistent_pretrigger_active", False)
        )
        persistent_pretrigger_label = resumed_state.get(
            "persistent_pretrigger_label"
        )
        print("Resumed waffle_v3 runtime state from previous run.")
    imminence_runtime = _load_imminence_runtime(imminence_model_path, imminence_state_model_path)
    pretrigger_imminence_runtime = _load_imminence_runtime(
        imminence_pretrigger_model_path, "", cache_slot="pretrigger"
    )
    plot_imminence_risk_history(
        ...
        color_arr,
        label,
        imminence_alert_avg_threshold
        if imminence_alert_avg_threshold is not None
        else imminence_alert_threshold,
        small_area_alert_threshold=imminence_small_area_alert_avg_threshold,
        large_area_alert_threshold=imminence_large_area_alert_avg_threshold,
        focus_label=None,
    )
    ...
    if region_source == "solarmonitor":
        last_solarmonitor_day = query_start_ut.astimezone(
    ...
    try:
        while stream_forever or time_diff <= duration_stream:
            ...
            if idx >= 0:
                ...
                cycle_day_yyyymmdd = start_time_series.astimezone(
                    datetime.timezone.utc
                ).strftime("%Y%m%d")
                if (
                    region_source == "solarmonitor"
                    and solarmonitor_refresh_on_utc_day_rollover
                ...
                xrsb_derivative = _latest_xrsb_derivative(xrsb_current)
                if imminence_runtime and imminence_runtime.get("goes_aux_cols"):
                    goes_aux_row = _build_runtime_goes_aux_row(
                        xrsa_current,
                        xrsb_current,
                        start_time_series,
                        imminence_runtime.get("goes_aux_prefix", "goes"),
                        imminence_runtime.get("goes_aux_lead_min", 0.0),
                    )
                    goes_aux_history.append(goes_aux_row)
                    if len(goes_aux_history) > 32:
                        del goes_aux_history[:-32]
                if print_phase_timing:
                ...
                pretrigger_states = [False] * n_ar
                cycle_new_trigger = False
                cycle_new_pretrigger = False
                ...
                if imminence_runtime:
                    ...
                    for lab in imminence_pretrigger_cooldown_remaining:
                        if imminence_pretrigger_cooldown_remaining[lab] > 0:
                            imminence_pretrigger_cooldown_remaining[lab] -= 1
                    recent_summary = []
                    for i in range(n_ar):
                        box_label = label[i]
                        risk_val, prob_val = _infer_imminence_risk_from_history(
                            vis_history[box_label], imminence_runtime, em_history[box_label], goes_aux_history
                        )
                        pretrigger_risk_val = np.nan
                        if pretrigger_imminence_runtime:
                            pretrigger_risk_val, _ = _infer_imminence_risk_from_history(
                                vis_history[box_label],
                                pretrigger_imminence_runtime,
                                em_history[box_label],
                                goes_aux_history,
                            )
                        if prob_val is not None:
                            ...
                            risk10_val = _extract_10m_probability(
                                prob_val, imminence_runtime.get("bins", [])
                            )
                            if np.isfinite(risk10_val):
                                risk10_history[box_label].append(float(risk10_val))
                                if len(risk10_history[box_label]) > 10:
                                    del risk10_history[box_label][:-10]
                        ...
                        if np.isfinite(pretrigger_risk_val):
                            pretrigger_risk_history[box_label].append(float(pretrigger_risk_val))
                            if len(pretrigger_risk_history[box_label]) > 10:
                                del pretrigger_risk_history[box_label][:-10]
                        ...
                        recent_pretrigger_vals = pretrigger_risk_history[box_label][-2:]
                        ...
                        if len(recent_vals) >= int(imminence_alert_count):
                            ...
                            if np.isfinite(area_frac) and area_frac <= 0.06:
                                avg_ok = float(np.mean(recent_vals)) >= float(imminence_small_area_alert_avg_threshold)
                            delta_ok = True
                            ...
                            em_ok = True
                            area_ok = True
                            physical_trend_ok = True
                            em_prev = float("nan")
                            em_curr = float(em_totals[i])
                            prev_area_frac = float("nan")
                            if (
                            ...
                            if len(active_area_fraction_history[box_label]) >= 2:
                                prev_area_frac = float(active_area_fraction_history[box_label][-2])
                            if imminence_min_active_area_fraction is not None:
                                area_ok = area_frac >= float(imminence_min_active_area_fraction)
                            if (
                                np.isfinite(em_prev)
                                and np.isfinite(em_curr)
                                and np.isfinite(prev_area_frac)
                                and np.isfinite(area_frac)
                                and em_curr < em_prev
                                and area_frac < prev_area_frac
                            ):
                                physical_trend_ok = False
                            avg_recent = float(np.mean(recent_vals))
                            peak_recent = float(np.max(recent_vals))
                            large_area_gate = (
                                np.isfinite(area_frac)
                                and area_frac >= float(imminence_large_area_fraction)
                            )
                            ...
                            em_jump_ok = (
                                ...
                                and np.isfinite(em_curr)
                                and em_prev > 0.0
                                and (em_curr / em_prev) >= float(imminence_pretrigger_em_jump_ratio)
                            )
                            if large_area_gate:
                                avg_ok = (
                                    np.isfinite(large_avg_recent)
                                    and large_avg_recent >= float(imminence_large_area_alert_avg_threshold)
                                )
                                peak_ok = peak_recent >= float(imminence_large_area_peak_threshold)
                                em_ok = em_ok and (
                                    np.isfinite(em_prev)
                                    and np.isfinite(em_curr)
                                    and em_prev > 0.0
                                    and (em_curr / em_prev) >= float(imminence_large_area_em_ratio_threshold)
                                )
                            pre_area_ok = np.isfinite(area_frac) and area_frac >= 0.02
                            pretrigger_ok = (
                                len(recent_pretrigger_vals) >= 2
                                and float(np.mean(recent_pretrigger_vals))
                                >= float(imminence_pretrigger_avg10_threshold)
                                and physical_trend_ok
                                and em_jump_ok
                                and pre_area_ok
                            )
                            xrs_rising_ok = (
                                (not imminence_require_positive_xrsb_derivative)
                                or (np.isfinite(xrsb_derivative) and xrsb_derivative > 0.0)
                            )
                            triggered = (
                                avg_ok
                                and peak_ok
                            ...
                            if i == focus_idx:
                                box_trigger_cooldown = int(imminence_alert_cooldown_remaining[box_label])
                                box_pretrigger_cooldown = int(imminence_pretrigger_cooldown_remaining[box_label])
                                if pretrigger_ok and not triggered and box_trigger_cooldown == 0 and box_pretrigger_cooldown == 0:
                                    pretrigger_states[i] = True
                                    cycle_new_pretrigger = True
                                    imminence_pretrigger_cooldown_remaining[box_label] = max(
                                        0, int(imminence_pretrigger_cooldown_frames)
                                    )
                                    trigger_txt = "PRETRIGGER10"
                                elif pretrigger_ok and not triggered and box_trigger_cooldown > 0:
                                    trigger_txt = f"cooldown({box_trigger_cooldown},box={box_label})"
                                elif pretrigger_ok and not triggered and box_pretrigger_cooldown > 0:
                                    trigger_txt = f"pretrigger_cooldown({box_pretrigger_cooldown},box={box_label})"
                                elif triggered and box_trigger_cooldown == 0:
                        ...
                        if vals:
                            ...
                            avg_pretrigger_recent = (
                                float(np.mean(recent_pretrigger_vals))
                                if recent_pretrigger_vals
                                else float("nan")
                            )
                            ...
                            msg = (
                                ...
                                f"gru10m_avg2={avg_pretrigger_recent:.3f} "
                            ...
                            if np.isfinite(xrsb_derivative):
                                msg += f"xrsb_d1={xrsb_derivative:.3e} "
                            else:
                                msg += "xrsb_d1=NA "
                            msg += f"-> {trigger_txt}"
                            print(msg)
                    ...
                    if cycle_new_trigger:
                        ...
                        if (
                            ...
                            imminence_trigger_times.append(
                                {
                                    "time": trigger_time_local,
                                    "label": trigger_label,
                                    "color": trigger_color,
                                    "linestyle": "-",
                                }
                    ...
                    if any(trigger_states):
                        ...
                        persistent_pretrigger_active = True
                        persistent_pretrigger_label = focus_label
                    elif persistent_pretrigger_active:
                        tracked_vals = pretrigger_risk_history.get(
                            persistent_pretrigger_label or "", []
                        )
                        tracked_recent = tracked_vals[-int(imminence_alert_count) :]
                        tracked_avg = (
                            float(np.mean(tracked_recent))
                            if len(tracked_recent) >= int(imminence_alert_count)
                            else float("nan")
                        )
                        if not np.isfinite(tracked_avg) or tracked_avg < persistent_pretrigger_release_avg:
                            persistent_pretrigger_active = False
                            persistent_pretrigger_label = None
                    plot_imminence_risk_history(
                        latest_plots_folder,
                ...
                plot_em_maps_and_curves(
                    ...
                    alert_face_mode=(
                        ...
                        if any(trigger_states)
                            ...
                            "pretrigger"
                            if persistent_pretrigger_active
                            else "watch"
                        )
                    ),
                    goes_anchor_time_local=goes_anchor_local,
                    goes_lead_minutes=goes_lead_minutes,
                )
                if print_phase_timing:
                ...
                if cycle_trigger_record is not None:
                    ...
                    trigger_folder = save_flare_trigger_snapshot(
                        ...
                        alert_config={
                            ...
                            "small_area_avg_threshold": imminence_small_area_alert_avg_threshold,
                            "pretrigger_avg10_threshold": imminence_pretrigger_avg10_threshold,
                            ...
                            "pretrigger_cooldown_frames": int(
        ...
        try:
            ...
            print(f"Failed to save waffle_v3 runtime state: {exc}")
